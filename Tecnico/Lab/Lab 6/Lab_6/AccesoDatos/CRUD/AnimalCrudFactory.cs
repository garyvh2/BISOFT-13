...
namespace AccesoDatos.CRUD
{
    public class AnimalCrudFactory : CrudFactory
    {
        ...
        public T RetrieveByNombre<T>(BaseEntity entity)
        {
            var lstResult = dao.ExecuteQueryProcedure(mapper.GetRetriveByNombreStatement(entity));
            var dic = new Dictionary<string, object>();
            if (lstResult.Count > 0)
            {
                dic = lstResult[0];
